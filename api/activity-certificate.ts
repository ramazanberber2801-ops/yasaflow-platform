...
import { applyHtmlSecurityHeaders } from './security-headers.js';
...
export default async function handler(req:VercelRequest,res:VercelResponse){
  ...
  const {data,error}=await supabase.from('activity_registrations')
    .select('id,full_name,activity_id,certificate_issued_at,status,activity:organization_activities(id,title,activity_date,location,certificate_title,certificate_signature_name,organization:organizations(name,logo_url))')
