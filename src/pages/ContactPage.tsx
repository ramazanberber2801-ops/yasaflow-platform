    ...
    <div className="min-h-screen pb-28" style={{ backgroundColor: brand.background, color: brand.text }}>
      <section className="relative h-[32vh] min-h-[200px] overflow-hidden">
        <div className="absolute inset-0">
          ...
          <div
            ...
            style={{
              background: `linear-gradient(to bottom, ${mix(brand.secondary, 40)}, ${mix(brand.secondary, 80)})`,
            }}
          />
        </div>
        ...
        <div className="rounded-2xl shadow-lg border-2 overflow-hidden" style={lightCardStyle}>
          <div className="relative h-36 overflow-hidden">
            ...
            <div
              className="absolute inset-0"
              style={{
                background: `linear-gradient(to bottom, ${mix(brand.secondary, 20)}, ${mix(brand.secondary, 40)})`,
              }}
            />
          ...
          <div className="p-5">
            <p className="text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color: brand.primary }}>
              FİZİKSEL KONUM
            </p>
      ...
      <section className="px-4 mt-5">
        <div className="rounded-xl border-2 shadow-md p-5" style={lightCardStyle}>
          ...
          <button
            ...
            style={{
              backgroundColor: notificationsEnabled ? brand.secondary : brand.primary,
        ...
        <div className="rounded-xl border-2 shadow-md overflow-hidden" style={lightCardStyle}>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-serif text-lg">Hocaya Sor</h2>
                <p className="text-xs opacity-80">Dini sorularınızı WhatsApp üzerinden sorun</p>
              </div>
            </div>
          </div>
          <div className="p-5">
      ...
      <section className="px-4 mt-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="font-serif text-lg">Dernek Kadromuz</h2>
        </div>

        {staff.length === 0 ? (
          <div className="rounded-xl p-8 text-center border-2" style={lightCardStyle}>
            <User size={32} className="mx-auto mb-2" style={{ color: mix(brand.primary, 40) }} />
            <p className="text-sm opacity-50">Henüz kayıtlı personel yok.</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {staff.map((member) => (
              <div
                key={member.id}
                className="rounded-xl p-4 shadow-sm border-2 flex items-center gap-4 hover:shadow-md transition-all"
                style={lightCardStyle}
              >
                <div
                  className="w-12 h-12 rounded-full flex items-center justify-center shrink-0"
                  style={{
                    background: `linear-gradient(135deg, ${brand.primary}, ${mix(brand.primary, 80, '#000')})`,
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-serif text-sm truncate">{member.name}</h3>
                  <p className="text-xs font-medium" style={{ color: brand.primary }}>{member.position}</p>

                  {member.phone && (
                    <a
                      href={`tel:${member.phone.replace(/\s/g, '')}`}
                      onClick={() => trackEvent('contact_click', member.id, member.name)}
                      className="text-xs opacity-50 transition-colors flex items-center gap-1 mt-1"
                    >
                      <Phone size={11} />
                      {member.phone}
                    </a>
