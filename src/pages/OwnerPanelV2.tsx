...
export function OwnerPanelV2() {
  ...
  useEffect(() => {
    if (isCreating) { setModules(defaultModules.map((mod) => ({ ...mod }))); return; }
    setModules(defaultModules.map((mod) => ({ ...mod })));
    if (!supabase || !organization.id) return;
    let cancelled = false;
    supabase.from('organization_modules').select('module_id, enabled, status').eq('organization_id', organization.id).then(({ data, error }) => {
      if (cancelled || error || !data) return;
      const savedModules = new Map(data.map((row) => [row.module_id, row]));
      setModules(defaultModules.map((mod) => {
        if (mod.core) return { ...mod, enabled: true, status: 'Inkludert' };
        if (mod.later) return { ...mod, enabled: false, status: 'Senere' };
  ...
  const persistOrganization = async () => {
    ...
    const { error: stepError } = await supabase.from('organization_provisioning_steps').upsert([
      { step_key: 'order_received', label: 'Bestilling mottatt', status: 'done' }, { step_key: 'organization_created', label: 'Organisasjon opprettet', status: 'done' }, { step_key: 'admin_ready', label: 'Admin klar', status: inviteState === 'sent' ? 'invited' : org.adminEmail ? 'pending' : 'waiting' }, { step_key: 'theme_selected', label: 'Tema valgt', status: org.themeId ? 'done' : 'pending' }, { step_key: 'published', label: 'Publisering', status: org.onboardingStep === 'Klar' ? 'done' : 'pending' },
