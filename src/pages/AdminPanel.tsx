        ...
        {tabs.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`flex-1 min-w-[78px] flex items-center justify-center gap-1.5 py-3 text-xs font-medium relative ${tab === t.id ? 'text-[#C5A880]' : 'text-[#2D2A26]/40'}`}
            >
              <Icon size={15} />
              <span>{t.label}</span>
...
function StaffManager({ items, onAdd, onUpdate, onDelete }: any) {
  ...
  return <div className="p-4"><AddButton label="Yeni Yönetim Üyesi Ekle" onClick={() => setShowForm(true)} />{items.length === 0 ? <EmptyState text="Henüz yönetim üyesi eklenmemiş." /> : <div className="space-y-3">{items.map((item: any) => <div key={item.id} className="bg-white rounded-xl p-3 border-2 border-[#C5A880]/25 flex items-center gap-3"><div className="w-12 h-12 rounded-full bg-[#C5A880] flex items-center justify-center text-white font-serif">{item.name?.charAt(0)}</div><div className="flex-1 min-w-0"><h3 className="font-serif text-sm truncate">{item.name}</h3><p className="text-xs text-[#C5A880]">{item.position}</p>{item.phone && <p className="text-xs text-[#2D2A26]/50">{item.phone}</p>}</div><ActionButtons onEdit={() => setEditing(item)} onDelete={() => { if (confirm('Bu yönetim üyesini silmek istiyor musunuz?')) onDelete(item.id); }} /></div>)}</div>}</div>;
}
...
function PushManager() {
  ...
  return <div className="p-4 space-y-4"><h2 className="font-serif text-xl">Toplu Bildirim Gönder</h2><div className="bg-white rounded-xl p-4 border-2 border-[#C5A880]/25 space-y-4"><input className={inputClass} placeholder="Başlık" value={title} onChange={e => setTitle(e.target.value)} /><textarea rows={5} className={`${inputClass} resize-none`} placeholder="Mesaj" value={message} onChange={e => setMessage(e.target.value)} /><button type="button" onClick={sendPush} disabled={sending} className="w-full py-3 rounded-lg bg-[#C5A880] text-white font-medium">{sending ? 'Gönderiliyor...' : '📢 Bildirim Gönder'}</button></div></div>;
}
...
function StatsManager() {
  ...
  return <div className="p-4 space-y-5"><h2 className="font-serif text-xl">İstatistik</h2>{sections.map(section => <div key={section.title} className="bg-white rounded-xl border-2 border-[#C5A880]/20 p-4"><h3 className="font-serif text-lg mb-3 text-[#2D2A26]">{section.title}</h3><div className="grid grid-cols-2 gap-3">{cards.map(card => <div key={card.key} className="bg-[#FAF6F0] rounded-xl p-3"><div className="text-xl mb-1">{card.icon}</div><p className="text-[11px] text-[#2D2A26]/50">{card.label}</p><p className="font-serif text-xl text-[#2D2A26]">{countForRange(card.key, section.days)}</p></div>)}</div></div>)}</div>;
}
