...
type OverviewStats = { total:number; active:number; trial:number; paused:number; pendingInvitations:number; readyToPublish:number };
const emptyStats:OverviewStats={total:0,active:0,trial:0,paused:0,pendingInvitations:0,readyToPublish:0};
...
export function OwnerOverview() {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      if(cancelled)return;
      const organizations=(organizationsResult.data||[]).filter((org)=>org.organization_type!=='system'&&org.id!=='dtim');
      const organizationIds=new Set(organizations.map((org)=>org.id));
      const admins=(adminsResult.data||[]).filter((admin)=>organizationIds.has(admin.organization_id));
      setStats({
  ...
  return <section className="px-4 pt-4" style={{color:brand.text}}><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{cards.map(({label,value,icon:Icon})=><div key={label} className="rounded-2xl border bg-white p-4 shadow-sm" style={{borderColor:mix(brand.primary,18)}}><div className="mb-3 flex items-center justify-between gap-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl" style={{backgroundColor:mix(brand.primary,10),color:brand.primary}}><Icon size={17}/></div><span className="text-2xl font-semibold">{loading?'–':value}</span></div><p className="text-[11px] leading-tight opacity-60">{label}</p></div>)}</div></section>;
}
