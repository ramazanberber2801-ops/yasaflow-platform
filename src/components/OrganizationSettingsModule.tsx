...
export function OrganizationSettingsModule({organizationId}:{organizationId:string}){
  ...
  if(loading)return <div className="flex justify-center p-8" aria-live="polite"><Loader2 className="animate-spin" aria-hidden="true"/></div>;
  ...
  return <form onSubmit={save} className="space-y-4">
    ...
    <section className="rounded-3xl border bg-white p-5 shadow-sm">
      ...
      <div className="relative mt-4">
        <button type="button" aria-label={t('settings.appLanguage')} aria-expanded={languageOpen} aria-controls="organization-language-options" onClick={()=>setLanguageOpen(!languageOpen)} className="flex w-full items-center justify-between rounded-xl border bg-white px-4 py-3 text-left"><span><span className="block text-sm font-semibold">{selectedLanguage.nativeName}</span><span className="block text-xs opacity-50">{selectedLanguage.name} · {selectedLanguage.code.toUpperCase()}{selectedLanguage.direction==='rtl'?' · RTL':''}</span></span><ChevronDown size={18} className={`transition ${languageOpen?'rotate-180':''}`}/></button>
        {languageOpen&&<div id="organization-language-options" className="absolute left-0 right-0 z-30 mt-2 rounded-2xl border bg-white p-3 shadow-2xl"><div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 opacity-35"/><input autoFocus aria-label={t('settings.searchLanguage')} value={languageQuery} onChange={e=>setLanguageQuery(e.target.value)} placeholder={t('settings.searchLanguage')} className="w-full rounded-xl border py-3 pl-9 pr-3 text-sm outline-none"/></div><div className="mt-2 max-h-64 overflow-y-auto">{filteredLanguages.length===0?<p className="p-4 text-center text-sm opacity-50">{t('settings.noLanguage')}</p>:filteredLanguages.map(language=><button key={language.code} type="button" onClick={()=>{setForm({...form,language:language.code});setLanguageOpen(false);setLanguageQuery('');}} className="flex w-full items-center justify-between rounded-xl px-3 py-3 text-left hover:bg-black/5"><span><span className="block text-sm font-semibold">{language.nativeName}</span><span className="block text-xs opacity-50">{language.name} · {language.code.toUpperCase()}{language.direction==='rtl'?' · RTL':''}</span></span>{form.language===language.code&&<Check size={17} style={{color:'var(--brand-primary)'}}/>}</button>)}</div></div>}
      </div>
    ...
    {error&&<p role="alert" className="rounded-xl bg-red-50 p-3 text-xs text-red-700">{error}</p>}{message&&<p role="status" aria-live="polite" className="rounded-xl bg-green-50 p-3 text-xs text-green-700">{message}</p>}
