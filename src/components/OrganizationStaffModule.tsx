...
export function OrganizationStaffModule({ organizationId }:{ organizationId:string }) {
  ...
  const uploadImage=async(event:ChangeEvent<HTMLInputElement>)=>{
    ...
    if(!file.type.startsWith('image/'))return setError('Velg en bildefil.');
    if(file.size>5*1024*1024)return setError('Bildet kan ikke være større enn 5 MB.');
  ...
  const move=async(index:number,direction:-1|1)=>{
    ...
    if(first.error||second.error){setError((first.error||second.error)?.message||'Rekkefølgen kunne ikke lagres.');await load();}
  };
  ...
  return <div className="space-y-4">
    ...
    {open&&<div className="fixed inset-0 z-[130] flex items-end bg-black/45 sm:items-center sm:justify-center sm:p-4"><form onSubmit={save} className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-3xl bg-white p-5 sm:rounded-3xl"><div className="flex justify-between"><h3 className="font-serif text-xl">{editing?'Rediger person':'Ny person'}</h3><button type="button" onClick={()=>setOpen(false)}><X/></button></div><div className="mt-4 space-y-3">
      ...
      <div className="rounded-2xl border p-4"><p className="text-sm font-semibold">Bilde (valgfritt)</p><div className="mt-3 flex items-center gap-3">{form.image_url?<img src={form.image_url} alt="" className="h-16 w-16 rounded-full object-cover"/>:<div className="flex h-16 w-16 items-center justify-center rounded-full bg-black/5"><Users size={22}/></div>}<label className="flex cursor-pointer items-center gap-2 rounded-xl border px-4 py-3 text-sm"><ImagePlus size={16}/>{uploading?'Laster opp...':'Velg bilde'}<input type="file" accept="image/*" className="hidden" disabled={uploading} onChange={event=>void uploadImage(event)}/></label>{form.image_url&&<button type="button" onClick={()=>setForm({...form,image_url:''})} className="text-xs text-red-700">Fjern</button>}</div></div>
      ...
      <div className="rounded-2xl border p-4"><p className="text-sm font-semibold">Kontaktmuligheter</p><p className="mt-1 text-xs opacity-55">Bare valgte kontaktmetoder vises på personens side.</p><div className="mt-3 grid grid-cols-2 gap-3 text-sm"><label className="flex items-center gap-2"><input type="checkbox" checked={form.allow_call} onChange={e=>setForm({...form,allow_call:e.target.checked})} disabled={!form.phone.trim()}/>Ring</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.allow_sms} onChange={e=>setForm({...form,allow_sms:e.target.checked})} disabled={!form.phone.trim()}/>SMS</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.allow_whatsapp} onChange={e=>setForm({...form,allow_whatsapp:e.target.checked})} disabled={!(form.whatsapp_number||form.phone).trim()}/>WhatsApp</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.allow_email} onChange={e=>setForm({...form,allow_email:e.target.checked})} disabled={!form.email.trim()}/>E-post</label></div></div>
      <div className="grid gap-3 rounded-2xl border p-4 text-sm"><label className="flex items-center gap-2"><input type="checkbox" checked={form.public_visible} onChange={e=>setForm({...form,public_visible:e.target.checked})}/>Vis personen offentlig i appen</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.active} onChange={e=>setForm({...form,active:e.target.checked})}/>Aktiv</label></div>
      {error&&<p className="text-xs text-red-700">{error}</p>}
      <button disabled={busy||uploading} className="flex w-full justify-center rounded-xl py-3 text-sm font-medium disabled:opacity-50" style={{background:'var(--brand-primary)',color:'var(--brand-primary-text)'}}>{busy?<Loader2 className="animate-spin" size={16}/>:editing?'Lagre endringer':'Opprett'}</button>
    </div></form></div>}
  </div>;
}
